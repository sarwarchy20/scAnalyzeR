...
server <- function(input, output,session) {
 ...
 # output$inst<-renderText({
  ...
  new2_data<-reactive({
   ...
   # withProgress(message = 'Filtering', value = 0, {
    ...
    else 
    {
      ...
      pbmc.data_2 <- Read10X(data.dir = "E:/seurat/Seurat_3 doc/www/filtered_gene_bc_matrices/hg19/")
  ...
  output$text <- renderUI({
    ...
    str1<-paste("Summary of Uploaded Dataset:")
  ...
  pbmc<- reactive({
    ...
    pbmct<- CreateSeuratObject(counts = new2_data()[,2:total_col], 
   ...
   # Sys.sleep(0)
    ...
    {
    pbmct[["percent.mt"]] <- PercentageFeatureSet(object = pbmct, pattern = "^MT-")
    pbmct
    }
    })
  })
...
# ....................Show Filtered Gene(mito genes)..............
  ...
  output$downloadData1<-downloadHandler(
    ...
    content = function(file) {
      write.csv(GetAssayData(object = pbmc())[mito.genes,], file, row.names = FALSE, na = "")
    }
  )
  # plot gene summary
  ...
  output$fdata_umi_gene<-renderPlot({
    if(input$th == FALSE) return()
...
#.................... Highly Variable Genes.................................
  ...
  output$hvg_list<-renderText({
    ...
    ##temp
    print(length(x= VariableFeatures(object = hv_pbmc())))
  ...
  output$hvg_download<- downloadHandler(
    ...
    content = function(file) {
      ...
      write.csv(as.matrix(hv_data), file, row.names = FALSE, na = "")
    }
  )
  ...
  pc_pbmc<-reactive({
   ...
   isolate({
     ...
     if(input$pc_genes == "pc_hvg")
     {
       ...
       RunPCA(object = scdata(), features = VariableFeatures(object = hv_pbmc()),
...
# print PCA
 ...
 # pdata <- ProjectPCA(object = pc_pbmc(), do.print = T)
  ...
  jplot <- JackStraw(object = pc_pbmc(), num.replicate = 100)
...
# PCA Heatmap
 output$pc_hplot<-renderPlot({
   ...
   isolate({
    ...
    DimHeatmap(object = pc_pbmc(), dims = input$pc_low:input$pc_upper, cells = input$pc_hmap_cells,
 ...
 output$DE_clus_UI <- renderUI({
   ...
   tabsetPanel( id = "de_tabset",
                ...
                tabPanel("Find all markers",
                         ...
                         sidebarPanel(width = 6, height = 1000, 
                                      ...
                                      #.....................
                                      checkboxInput('de_filter',p('Filtering Markes')),
                                      conditionalPanel('input.de_filter == 1',
                                                       ...
                                                       checkboxInput('de_filter_show',p('Show list')),
                ...
                tabPanel("Find markers by cluster",
                         img(src = "line_font1.png"),
                         sidebarPanel(width = 6,"Please select parameters:",
                                      #tags$hr(),
                                      br(),
                                      tags$head(
                ...
                tabPanel("Find markers by clusters vs other clusters",
                         ...
                         sidebarPanel(width = 6, "Please select parameters:",
                                      ...
                                      column(width = 12, textAreaInput("clst1","Select Cluster/s", "The cluster number must be separated by a commma")),
                                      ...
                                      tags$hr(),
                                      br(),
                                      column(width = 12, textAreaInput("clst2","Select complementary cluster/s", "The cluster number must be separated by a commma")),
...
input_paths<- reactive({
  ...
  if(input$path_source == "path_KEGG")
    ...
    kegg_pathways_t <- mappedkeys(KEGGPATHID2EXTID) %>% str_subset('hsa') # maps KEGG pathway identifiers to Entrez Gene; 'hsa' for human and 'mmu' for mouse
    #kegg_pathways_t
    
    kegg_pathways<- kegg_pathways_t%>% str_replace(sprintf('^%s(.*)$', 'hsa'), '\\1')
    #kegg_pathways
 ...
 path_genes<-reactive({
   ...
   if(input$path_hmap_genes == "path_hmap_genes_sig")
   {
     ...
     syms_sig=eg2id(tlp[[1]], org="Hs", category ="symbol") # convert gene entrez ID to gene symbol 
     #length(syms_sig[,2])
...
#--------------------------------------------------------------------- Dynamic Heatmap -------------------------------------
 output$dhmap_input_UI<- renderUI({
   ...
   {
     textAreaInput("dh_map_genelist","Gene List", "Please write gene sybmols: one gene per line")
