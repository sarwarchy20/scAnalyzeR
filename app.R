...
server <- function(input, output,session) {
  ...
  new2_data<-reactive({
    ...
    if(input$sor_data == "upd_data")
    {
      ...
      incProgress(1, message = "Please wait..." ,  
                  detail = paste("Completed ", i*100,"%"))
    ...
    else if(input$sor_data == "data_10x")
    {
      ...
      withProgress(message = 'Uplooading...', value = 0, {
  ...
  output$pc_jac_show<-renderPlot({
    ...
    incProgress(1, message = "Please wait...",
                detail = paste("In progress... ", i*100,"%"))
    Sys.sleep(3)
  }
})
  ...
  output$DE_clus_UI <- renderUI({
    ...
    tabsetPanel(id = "de_tabset",
                ...
                tabPanel("Find all markers",
                         ...
                         conditionalPanel('input.de_filter_hmap==1', 
                ...
                tabPanel("Find markers by cluster",
                         ...
                         conditionalPanel('input.sde_filter_hmap_cluster==1',
  ...
  all_markers<-reactive({
    ...
    shiny::isolate({
      ...
      withProgress(message = 'Finding all markers... please wait!', value = 0, {
        
        n <- 1
        
        for (i in 1:n) {
      
      all_mk<- FindAllMarkers(clus_pbmc(), assay = NULL, features = NULL,
  ...
  cluster_markers<-reactive({
    ...
    shiny::isolate({
      ...
      withProgress(message = 'Find markers... please wait', value = 0, {
  ...
  output_paths <-  reactive({
    ...
    shiny::isolate({
      ...
      withProgress(message = 'Compute pathways...please wait!', 
                   value = 0, {
  ...
  output$path_hmap_show<- renderPlot({
    ...
    DoHeatmap(clus_pbmc(), features = path_genes() , group.bar = T) +
      scale_fill_gradientn(colours = redgreen(255)) +
